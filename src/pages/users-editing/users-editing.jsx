import { useEffect, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { fetchUsers, removeUser, fetchRoles } from '../../bff/operations';
import { setIsLoading } from '../../actions';
import { AccessDenied, H2, Loader, MainBlock } from '../../components';
import { TableRow, UserRow } from './components';
import { selectIsLoading, selectUserRole } from '../../selectors';
import styled from 'styled-components';
import { ROLE } from '../../bff/constants';

const UsersEditingContainer = ({ className }) => {
	const [usersFromServer, setUsersFromServer] = useState([]);
	const [roles, setRoles] = useState([]);
	const [shouldUpdateUserList, setShouldUpdateUserList] = useState(false);

	const userRole = useSelector(selectUserRole);
	const isLoading = useSelector(selectIsLoading);

	const isAdmin = userRole === 0 ? true : false;

	const dispatch = useDispatch();

	useEffect(() => {
		fetchUsers().then(({ res }) => setUsersFromServer(res));
		fetchRoles().then(({ res }) => setRoles(res));

		dispatch(setIsLoading(false));
	}, [dispatch, shouldUpdateUserList]);

	const deleteUser = (userId) => {
		removeUser(userId).then(() => {
			setShouldUpdateUserList(!shouldUpdateUserList);
		});
	};

	return (
		<MainBlock>
			{isLoading ? (
				<Loader />
			) : isAdmin ? (
				<div className={className}>
					<H2>Пользователи</H2>
					<div className="users-block">
						<TableRow />
						{usersFromServer.map(({ id, login, roleId }) => (
							<UserRow
								key={id}
								id={id}
								login={login}
								roleId={roleId}
								deleteUser={deleteUser}
								roles={roles.filter(({ id }) => id !== ROLE.GUEST)}
							/>
						))}
					</div>
				</div>
			) : (
				<AccessDenied children="Доступ только для администратора." />
			)}
		</MainBlock>
	);
};

export const UsersEditing = styled(UsersEditingContainer)`
	width: 100%;
	padding: 15px 20px;
	display: flex;
	justify-content: center;
	flex-direction: column;

	h2 {
		text-align: center;
	}

	.users-block {
		width: 600px;
		min-height: 200px;
		margin: 20px auto;
		border-radius: 7px;
		border: 1px solid #d9d9d9;
		color: #5c5740;
	}
`;
